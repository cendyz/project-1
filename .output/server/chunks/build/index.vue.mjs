  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(grid_data), (item, index) => {
        _push(`<div class="${ssrRenderClass([[item.my_class, item.img ? "lg:flex-row " : "lg:flex-col"], "border-[2px] border-primary-1 rounded-3xl lg:flex lg:items-center overflow-hidden sm:mb-[3.5rem] lg:mb-0"])}"><div class="p-[2rem] lg:m-auto lg:text-left"><h3 class="font-w700 mb-[2rem] text-[2rem]">${ssrInterpolate(item.title)}</h3><p class="">${ssrInterpolate(item.desc)}</p></div>`);
        if (item.img) {
