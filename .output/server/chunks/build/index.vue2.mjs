...
import { c as useRouter, u as useBankStore } from './server.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--[--><header class="pb-[5rem] pt-[11rem] dark:bg-neutral-21"><h1 class="uppercase text-primary-1 dark:text-primary-10 text-[5rem] px-[2rem] text-center font-w700">latest news</h1><div class="py-[1rem] border-t-[2px] border-b-[2px] border-primary-2 dark:bg-neutral-40 dark:text-primary-10 bg-neutral-4 overflow-hidden mt-[2rem]">`);
      ...
      _push(`</div></header><main class="bg-neutral-3 pt-[3rem] pb-[7rem] px-[3rem] grid gap-y-[3rem] lg:gap-y-[7rem] place-items-center dark:bg-neutral-21"><!--[-->`);
      ssrRenderList(unref(store).articles_data, (item, index) => {
        _push(`<div prefetch class="rounded-lg overflow-hidden bg-neutral-4 dark:bg-neutral-40 dark:text-primary-10 max-w-[40rem] lg:flex lg:max-w-[110rem] lg:cursor-pointer dark:border-[3px]"><img${ssrRenderAttr("src", item.img)}${ssrRenderAttr("alt", item.alt)} class="lg:w-[40rem] lg:h-[30rem]"><div class="p-[2rem]"><h2 class="font-w700 text-[2rem] lg:text-[3rem] lg:w-[40rem]">${ssrInterpolate(item.title)}</h2><p class="text-[1.4rem] mt-[1rem] lg:text-[1.6rem]">${ssrInterpolate(item.desc)}</p></div></div>`);
      });
      _push(`<!--]--></main>`);
