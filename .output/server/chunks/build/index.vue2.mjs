  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(store).articles_data, (item, index) => {
        _push(`<div prefetch class="rounded-lg overflow-hidden bg-neutral-2 dark:bg-neutral-40 dark:text-primary-10 max-w-[40rem] lg:flex lg:max-w-[110rem] lg:cursor-pointer border-[3px] border-transparent dark:border-neutral-4"><img${ssrRenderAttr("src", item.img)}${ssrRenderAttr("alt", item.alt)} class="lg:w-[40rem] lg:h-[30rem]"><div class="p-[2rem]"><h2 class="font-w700 text-[2rem] lg:text-[3rem] lg:w-[40rem]">${ssrInterpolate(item.title)}</h2><p class="text-[1.4rem] mt-[1rem] lg:text-[1.6rem]">${ssrInterpolate(item.desc)}</p></div></div>`);
      });
      _push(`<!--]--></main>`);
